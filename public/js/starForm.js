$(document).ready(function(){
	...
	$("#sub").on('click', function(e){
		...
		if ('' == $("#name").val().trim()) {
			layer.tips('必须填写明星姓名！', '#name', {tips: 1});
			return;
		};
		...
		if ('' == $("#pic").val().trim()) {
			layer.tips('必须添加明星图片！', '#pic', {tips: 1});
			return;
		};

		if ('' == _summary) {
			layer.tips('必须填写明星简介！', '#summary', {tips: 1});
			return;
		};
	...
	$("#t").on('click', function(){
		...
		$("#tagList").find('.tag:checked').each(function(){
			...
			html += '<span class="label label-info">'+tagName+'<input type="hidden" name="star[category]" value="'+tagId+'"></span>&nbsp;'
		})
