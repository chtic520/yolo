$(document).ready(function(){
	...
	$("#sub").on('click', function(e){
		...
		/*if ('' == $("#name").val().trim()) {
			layer.tips('必须填写网红姓名！', '#name', {tips: 1});
			return;
		};
		...
		if ('' == $("#pic").val().trim()) {
			layer.tips('必须添加网红图片！', '#pic', {tips: 1});
			return;
		};

		if ('' == _summary) {
			layer.tips('必须填写网红简介！', '#summary', {tips: 1});
			return;
		};*/
		  ...
		  $.ajax({
		  	...
		  	success: function(data){
		  		if (data.success === 1) {
		  			...
		  			$("#tagList").prepend('<div class="col-sm-2"><label><input class="tag" type="radio" name="pt" id="'+data.category._id+'"> <span>'+data.category.name+'</span></label></div>')
		  			layer.close(index);
		  		}
		  	}
		  })
	...
	$("#t").on('click', function(){
		...
		$("#tagList").find('.tag:checked').each(function(){
			...
			html += '<span class="label label-info">'+tagName+'<input type="hidden" name="wemedia[category]" value="'+tagId+'"></span>&nbsp;'
		})
	...
	$("#p").on('click', function(){
		var html = '';
		$("#platformList").find('.tag:checked').each(function(){
			var tagName = $(this).siblings('span').html();
			var tagId = $(this).attr('id');
			html += '<span class="label label-info">'+tagName+'<input type="hidden" name="wemedia[platform]" value="'+tagId+'"></span>&nbsp;'
		})

		$("#platformBox").html(html)
