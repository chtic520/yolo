$(document).ready(function(){
	...
	$('.i-switch').on('switchChange.bootstrapSwitch', function(e, state) {
	  ...
	  if (state && _onItems < 4) {
	  	...
	  	$.ajax({
	  		type: 'POST',
	  		url: '/admin/webstar/categorytag/display',
	  ...
	  }else if(state && _onItems >= 4){
	  	layer.msg('只能选择4条案列在前台展示！');
	...
	$("#update").on('click', function(){
		...
		if (rows.length == 0) {
			layer.alert('请选择一条媒体更新！', {icon: 0, title:'提示'});
		}else if(rows.length > 1){
			layer.alert('一次只能更新一条媒体！', {icon: 0, title:'提示'});
		}else{
	...
	$("#del").on('click', function(){
		...
		layer.confirm('确定要删除这' + rows.length + '条数据吗', {icon: 3, title:'提示'}, function(index){
		  $.ajax({
		  	type: 'DELETE',
		  	url: '/admin/webstar/categorytag/del',
